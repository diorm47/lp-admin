...
function Conclusions() {
  ...
  const refresh = (offset = 0) => {
    mainApi
      .getOutputsAction(offset)
      .then((res) => {
        setConclusions(res.results);
  ...
  const toggleAllDataSelected = () => {
    if (selected.length == conclusions.length) {
      setSelected([]);
    } else {
      setSelected([...conclusions]);
    }
  };
  ...
  const filterItems = (type) => {
    ...
    if (type !== "all") {
      const filtered = conclusions.filter((item) => item.status === type);
      setConclusions(filtered);
    } else {
      setConclusions(conclusions.slice(0, 10));
    }
  };
    ...
    <>
      ...
      <div className="template_page analytics_page">
        ...
        <div className="cases_top_togglers">
          <button className="main_btn" onClick={() => filterItems("all")}>
            <p>Все выводы</p>
          </button>
          <button
            className={
                  ...
                  <td className="users_select">
                    <div className="select_all">
                      <div className="is_selected ml_55px">
                        {selected.length == conclusions.length ? (
