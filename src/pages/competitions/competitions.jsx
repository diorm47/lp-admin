...
function Competitions() {
  ...
  const toggleAllDataSelected = () => {
    if (selected.length == competitions.length) {
      setSelected([]);
    } else {
      setSelected([...competitions]);
    }
  };
  ...
  useEffect(() => {
    getContests(currentPage * 10);
  }, [currentPage]);

  const getContests = (offset = 0) => {
    mainApi
      .getContestsAction(offset)
      .then((res) => {
                ...
                {competitions && competitions.length
                  ? competitions.map((competition) => (
                      <tr key={competition.contest_id}>
                        ...
                        <td className="tac">
                          {competition.count_participants}
                        </td>
                          ...
                          <div className="cases_table_actions">
                            <div className="cases_table_actions_list">
                              <div
                                ...
                                onClick={() =>
                                  editContest(competition.contest_id)
                                }
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
