...
function EditItem() {
    ...
    if (params.item) {
      mainApi
        ...
        .then((res) => {
          ...
          setItemPriceCrystals(res.crystals_quantity);
          setItemSellPrice(res.sale_price);
          setItemPercentPrice(res.percent_price);
          setSelectedRarity(res.rarity_category.rarity_id);
  ...
  const saveItem = () => {
    mainApi
      ...
      .then((res) => {
        console.log(res);
        snackbarActions("Предмет обновлён!");
      })
      .catch((error) => {
        console.log("error", error);
        snackbarActions("Ошибка обновления предмета!");
      });
  };
  useEffect(() => {
    mainApi
